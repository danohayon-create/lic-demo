...
export type RolePipelineStatus = 'New' | 'Viewed' | 'No Go' | 'Shortlisted' | 'Callback' | 'Offer' | 'Cast'
...
export const PIPELINE_STATUSES: RolePipelineStatus[] = [
  'New', 'Viewed', 'No Go', 'Shortlisted', 'Callback', 'Offer', 'Cast',
]
...
const seedCandidates: Candidate[] = [
  ...
  { id: 'cand-11', roleId: 'la-temoin-film', name: 'Lola Mercier',   age: 27, city: 'Marseille',  good: 2, maybe: 0, no: 0, status: 'shortlisted' },
  { id: 'cand-12', roleId: 'la-temoin-film', name: 'Zoé Andrieu',    age: 30, city: 'Nice',        good: 1, maybe: 1, no: 0, status: 'shortlisted' },

  // Fresh submissions — not reviewed yet, waiting in the New column
  { id: 'cand-13', roleId: 'chloe-marchand', name: 'Anaïs Roche',    age: 34, city: 'Bordeaux',   good: 0, maybe: 0, no: 0, status: 'new' },
  { id: 'cand-14', roleId: 'chloe-marchand', name: 'Lucie Fontaine', age: 39, city: 'Nantes',      good: 0, maybe: 0, no: 0, status: 'new' },
  { id: 'cand-15', roleId: 'remy-jourdain', name: 'Vincent Berry',   age: 44, city: 'Lille',       good: 0, maybe: 0, no: 0, status: 'new' },
...
const STORAGE_KEY = 'lic-selection-state-v5'
...
export function rateCandidate(candidateId: string, signal: 'good' | 'maybe' | 'no') {
  state = {
    ...
    candidates: state.candidates.map((c) => {
      ...
      // Auto-sort between No Go / Shortlisted while the candidate hasn't been
...
/** Reconstructs plausible individual votes from the good/maybe/no tally, for "other ratings" bubbles. */
export function deriveTeamRatings(c: Candidate): { initials: string; signal: 'good' | 'maybe' | 'no' }[] {
  const signals: ('good' | 'maybe' | 'no')[] = [
